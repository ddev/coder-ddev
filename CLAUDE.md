...
- `scripts/workspace-lifecycle-cleanup.sh` - Notifies then deletes idle workspaces; runs via systemd timer on each Coder server (see `scripts/workspace-lifecycle-cleanup.service`/`.timer`, must be installed on server)
