...
- Confirmation that the `freeform` template is available
...
coder create --template freeform my-first-workspace --yes

# Or with custom parameters
coder create --template freeform my-first-workspace \
  --parameter cpu=8 \
  --parameter memory=16 \
  --yes
```
...
**Startup time:** faster than initial create (no reinstall needed)
