...
# docs/drupal-core.html, an already-open browser tab) hard-fails `terraform
...
LAUNCH_EOF
    ...
    if [ "$SETUP_FAILED" = "true" ]; then
      ...
      log_setup "   - Click 'Drupal Site' in Coder dashboard"
...
BASHCOMP
    ...
    if [ "$SETUP_FAILED" = "true" ]; then
      ...
      echo ""
      echo "Next steps:"
      echo "  1. Click 'Drupal Site' app to access your site"
