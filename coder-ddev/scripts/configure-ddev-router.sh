...
if [ -n "${CODER_AGENT_URL:-}" ]; then
    echo "Extracting workspace info from CODER_AGENT_URL..."
    ...
    echo ""
    echo "Could not auto-detect from CODER_AGENT_URL."

    if [ -z "$WORKSPACE_ID" ]; then
        echo "Please enter your workspace ID (e.g., 955tu8k2ih1ck):"
        read -r WORKSPACE_ID
    fi
...
if [ -z "$WORKSPACE_NAME" ] || [ -z "$WORKSPACE_OWNER" ] || [ -z "$WORKSPACE_ID" ] || [ -z "$CODER_DOMAIN" ]; then
    ...
    echo "  export CODER_WORKSPACE_OWNER_NAME=your-username"
    echo "  export CODER_AGENT_URL=https://your-workspace-id.your-domain.com/"
    exit 1
fi
...
echo "✓ Created .ddev/config.coder.yaml"
echo ""
