...
COMPOSE_EOF
    ...
    if [ "$SETUP_FAILED" = "false" ]; then
      ...
      if [ "$DRUPAL_INSTALLED" = "false" ]; then
        ...
        if [ "$_poser_ok" = "true" ]; then
          # `ddev poser` scaffolds Drupal-core's own root-level .gitattributes
          # into this directory (alongside this module's own git repo). That
          # file's `eol=lf` rules retroactively make git treat any tracked
          # file with CRLF line endings (e.g. a module's .tugboat/config.yml)
          # as locally modified — purely a normalization artifact, not a real
          # edit. We're still on a pristine, just-checked-out tree at this
          # point (nothing has touched tracked files since Phase 3), so it's
          # safe to discard any such diff here rather than let it sit around
          # and later block an issue-branch checkout with a confusing
          # "local changes would be overwritten" error.
          _scaffold_dirty=$(git diff --name-only -- . ':!composer.json' 2>/dev/null || true)
          if [ -n "$_scaffold_dirty" ]; then
            log_setup "Restoring tracked files normalized by scaffolded .gitattributes:"
            echo "$_scaffold_dirty" | sed 's/^/  /' | tee -a "$SETUP_LOG"
            echo "$_scaffold_dirty" | xargs -r git checkout -- >> "$SETUP_LOG" 2>&1 || true
          fi
        fi
