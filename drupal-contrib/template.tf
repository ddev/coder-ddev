...
STATUS_HEADER
    ...
    #
    # .gitattributes / .editorconfig / .gitignore / recipes/ are scaffolded into
    # this same directory by `ddev poser` (Phase 5's composer install) — they
    # belong to the Drupal-core site scaffold, not to this module's own repo.
    # Excluding them keeps `git status` clean AND keeps the `git clean -fd`
    # below (and on any later re-checkout) from deleting them.
    mkdir -p .git/info
...
GIT_EXCLUDE_EOF
    ...
    # `ddev poser`'s composer scaffold also drops Drupal-core's own
    # .gitattributes into this directory, with `eol=lf` rules that apply to
    # ANY path git looks at here — including this module's own tracked files.
    # If the module committed a file with CRLF line endings (e.g. some
    # projects' .tugboat/config.yml), git then treats it as perpetually
    # "locally modified", which can block an issue-branch checkout later.
    # .git/info/attributes is never committed and takes precedence over any
    # in-tree .gitattributes (same relationship as .git/info/exclude to
    # .gitignore), so this neutralizes the scaffold's normalization for this
    # module's repo without touching the scaffold files themselves. Note:
    # `git checkout -- <path>` does NOT fix this on its own — checkout
    # re-applies the same eol=lf smudge filter on write, so without this
    # override it just recreates the mismatch it was meant to clear.
    cat > .git/info/attributes << 'GIT_ATTRIBUTES_EOF'
# Generated by coder-ddev workspace startup — do not edit.
* -text
GIT_ATTRIBUTES_EOF
    ...
    # Normalize: users often paste the full drupal.org fork slug
    # ("$PROJECT_NAME-1234567") instead of the bare issue number the
    # parameter description asks for. Strip a redundant project-name prefix
    # so we don't build a doubled, nonexistent fork URL like
    # ".../issue/$PROJECT_NAME-$PROJECT_NAME-1234567.git".
    case "$ISSUE_FORK" in
      "$PROJECT_NAME-"*)
        log_setup "Normalizing issue_fork '$ISSUE_FORK' -> '$${ISSUE_FORK#$PROJECT_NAME-}' (stripped redundant project prefix)"
        ISSUE_FORK="$${ISSUE_FORK#$PROJECT_NAME-}"
        ;;
    esac
    ...
    if [ "$SETUP_FAILED" = "false" ] && [ -n "$ISSUE_FORK" ]; then
      ...
      if git fetch "$REMOTE_NAME" >> "$SETUP_LOG" 2>&1; then
        ...
        log_setup "✗ Failed to fetch issue fork remote $FORK_URL"
        update_status "✗ Issue fork fetch: Failed"
        ISSUE_SETUP_OK=false
      fi
      ...
      if [ -n "$ISSUE_BRANCH" ]; then
        ...
        if [ "$CURRENT_BRANCH" != "$ISSUE_BRANCH" ]; then
          ...
          if git checkout "$ISSUE_BRANCH" >> "$SETUP_LOG" 2>&1 || \
            ...
            # Excluded dirs (.ddev, vendor, web — see .git/info/exclude and the
            # module's own scaffolded .gitignore) are additionally protected
            # here with -e so a later branch switch can't wipe out an
            # already-provisioned DDEV/Drupal install.
            git reset --hard HEAD >> "$SETUP_LOG" 2>&1 || true
            git clean -fd -e .ddev -e vendor -e web >> "$SETUP_LOG" 2>&1 || true
            log_setup "✓ Working tree reset to match branch"
          else
            log_setup "✗ Failed to checkout $ISSUE_BRANCH — remaining on $CURRENT_BRANCH"
            update_status "✗ Branch checkout: Failed"
            ISSUE_SETUP_OK=false
          fi
        else
    ...
    # ==========================================
    {
      ...
      if [ -n "$ISSUE_FORK" ]; then
        ...
        if [ "$ISSUE_SETUP_OK" = "false" ]; then
          ...
          echo "   You are on branch $(git -C "$PROJ_DIR" branch --show-current 2>/dev/null), not $ISSUE_BRANCH."
          echo "   The issue fork remote fetch or branch checkout failed — see:"
          echo "     cat $SETUP_LOG"
        fi
      fi
    } > ~/WELCOME.txt
